...
class SDFSampler(DiffusionSampler):
    ...
    def get_embeddings(
        ...
        if autoreg_cond is None:
            cond = -torch.ones(batch_size, 1, self.d_cond, device=x.device, dtype=x.dtype)
        else:
            cond = self.model.autoreg_cond_enc(autoreg_cond)
            
        e_t, embed = self.model(x,t,cond,True)
    ...
    @torch.no_grad()
    def findembed(self, clean_x, ts=0, seg_length=None, autoreg_cond=None, noise=None, batch_size=None):
        # clean_x is the song we'll input
        if batch_size is None:
            batch_size = clean_x.size(0)
        if seg_length is None:
            seg_len = clean_x.size(2)

        shape = [batch_size, self.out_channel, seg_len, self.h]
        
        #print(f"Shape={shape}")
        ts = clean_x.new_full((batch_size, ), ts, dtype=torch.long)
        input_x = self.q_sample(clean_x, ts, noise)

        return self.get_embeddings(input_x, ts, autoreg_cond)
