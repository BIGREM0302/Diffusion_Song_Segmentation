...
def load_model(path: Path = None) -> LatentDiffusion:
    ...
    # Load the checkpoint
    # the file on "path" is always .pt or .ckpt
    # move to CPU
    # torch.load() will return the object, always contains "state_dict": weights and biases, "globa_step": training step, "optimizer_state_dict": parameters for optimizer
    with monit.section(f"Loading model from {path}"):
        checkpoint = torch.load(path, map_location="cpu")

    # Set model state
    with monit.section('Load state'):
    # strict = False means don't report error if there is any excessive parameters or missing parameters
    ...
    # change to inference mode, shut down functions such as drop out, and turn batch norm into running average
    model.eval()
    return model
