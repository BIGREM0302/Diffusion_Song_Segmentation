...
def gather(consts: torch.Tensor, t: torch.Tensor):
    ...
    # consts: tensor with shape (time_step, ): 1-D array
    # t: tensor with shape (B, ): 1-D array, which corresponds to each image in the minibatch at which time state
    # x0: tensor with shape (B, C, H, W)
    # output = A.gather(dim, index), take A's dim dimension elements according to the index list
    c = consts.gather(-1, t) # c: tensor with shape (B, ) c = torch.tensor([0.3, 0.2, 0.1])
    return c.reshape(-1, 1, 1, 1) # c: tensor with shape (B, 1, 1, 1) c = torch.tensor([[[[0.3]]],[[[0.2]]],[[[0.1]]]])
...
class LatentDiffusion(nn.Module):
    ...
    * [U-Net](model/unet.html) with [attention](model/unet_attention.html)
    """
    eps_model: UNetModel # define an object eps_model which has the type of UNetModel
    first_stage_model: Optional[Autoencoder] = None # define an object can be none but also as type of Autoencoder
    ...
    def __init__(
        ...
        # freeze latent autoencoder's(not include condition encoder) parameters
        if self.first_stage_model is not None:
            for param in self.first_stage_model.parameters(): # neural network type has parameter() method
        ...
        # $\beta$ schedule
        # linear_start and linear_end are input parameters
        beta = torch.linspace(
        ...
        # store alpha, beta, and alpha_bar, and sigma as parameters
    ...
    # @property enables we call model:LatentDiffusion this method by model.device instead of model.device()
    @property
    def device(self):
        """
        ### Get model device
        """
        # Watch the first parameter on which device? i.e. which GPU
    ...
    def autoencoder_encode(self, image: torch.Tensor):
        ...
        """
        # if no auto encoder, then just return the image
        if self.first_stage_model is not None:
    ...
    def autoencoder_decode(self, z: torch.Tensor):
        ...
        """
        # if no auto encoder, then just return the result
        if self.first_stage_model is not None:
            return self.first_stage_model.decode(z / self.latent_scaling_factor)
        else:
            return z

    def forward(self, x: torch.Tensor, t: torch.Tensor, context: torch.Tensor, return_embedings=False):
        """
        ### Predict noise
        ...
        """
        # self.eps_model = unet_model
        # the output of this class is equivalent to unet
        return self.eps_model(x, t, context, return_embedings)
    ...
    def q_xt_x0(self, x0: torch.Tensor,
        ...
        """
        # i.e. this is the noising process, given x0, t we can give the distribution of xt (specified by mean and variance)
        # [gather](utils.html) $\alpha_t$ and compute $\sqrt{\bar\alpha_t} x_0$
        # dimension: output of gather = (B, 1, 1, 1), x0 = (B, C, H, W)
    ...
    def q_sample(
        ...
        # if not give this noise, then just randomly generate noise with the same shape as x0
        # dimension of epsilon will be the same as x0
        if eps is None:
            eps = torch.randn_like(x0)

        # get $q(x_t|x_0), q_xt_x0 will return mean and variance$
    ...
    def loss(
        ...
        #### Simplified Loss
        """
        # In fact, Union[torch.Tensor,None]=Optiona[torch.Tensor]
        # Get batch size, since x0: (B, C, H, W)
        batch_size = x0.shape[0]
        # Get random $t$ for each sample in the batch, data type is 64-bit integer, and constrct on the same device of x0
        t = torch.randint(
            0, self.n_steps, (batch_size, ), device=x0.device, dtype=torch.long
        )
        # if there is autoencoder, then encode x0 first
        if self.first_stage_model is not None:
        ...
        if autoreg_cond is None:
            # if no autoregressive condition, send all -1 tensor,with shape (batch_size, 1, d_cond)
        ...
        if external_cond is not None:
            ...
            # with probability 20 percent that will drop external condition
            if random.random() < 0.2:
                external_cond = (-torch.ones_like(external_cond)).to(self.device)
            # the external condition and autoregressive condition should have the same shape
            # concatenate two conditions along the "1st" dimension, that is, the channel
            # so now cond shape is (B, 2, d_cond)
            cond = torch.cat([autoreg_cond, external_cond], 1)
        else:
            cond = autoreg_cond
        
        # if x0 as the input, number of channel = output of UNet's channel, which means no background condition
        if x0.size(1) == self.eps_model.out_channels:  # generating form, since when generating form we won't have background condition
            ...
            # the difference between real noise and predicted noise
            loss = F.mse_loss(noise, eps_theta)
        else:
            if self.debug_mode:
                print('In the mode of non-root level:', x0.size(), cond.size(), autoreg_cond.size())
            # first two channel is for noising, why first two? Since reduce lead sheet, lead sheet, accompaniment are all with 2 channels
            if noise is None:
                noise = torch.randn_like(x0[:, 0: 2])
            # add noise to the first 2 channel
            front_t = self.q_sample(x0[:, 0: 2], t, eps=noise)
            # rest of channel is for background condition
            background_cond = x0[:, 2:]
            # concatentate as the collective input
