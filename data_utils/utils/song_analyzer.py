...
from .format_converter import note_matrix_to_piano_roll, chord_mat_to_chord_roll, note_end_matrix_to_piano_roll
...
class LanguageExtractor:
    ...
    def extract_pianoroll(self):
        piano_roll = note_end_matrix_to_piano_roll(self.song.mat, self.song.total_step)
        return piano_roll

    def extract_all_hie_langs(self):
        if self.song.acc is not None:
            ...
            piano_roll = self.extract_pianoroll()
            return {'roll':piano_roll}
    ...
    def analyze_for_training(self):
        ...
        if self.song.melody is not None:
            min_mel_pitch, max_mel_pitch = self.song.melody[:, 1].min(), self.song.melody[:, 1].max()
        else:
            min_mel_pitch, max_mel_pitch = self.song.mat[:,1].min(), self.song.mat[:,1].max()
