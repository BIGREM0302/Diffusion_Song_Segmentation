...
if __name__ == '__main__':
    ...
    output_fn0 = os.path.join('data', 'split0.npz')
    output_fn1 = os.path.join('data', 'split1.npz')
    create_train_valid_split(output_fn0, 909, train_factor=9, seed=1234)
    create_train_valid_split(output_fn1, 2656, train_factor=9, seed=1234)

    t_id, v_id = load_split_file(output_fn0)
    print(f'n_train={len(t_id)}, n_valid={len(v_id)}')
    print(t_id)
    print(v_id)
    t_id, v_id = load_split_file(output_fn1)
    print(f'n_train={len(t_id)}, n_valid={len(v_id)}')
    print(t_id)
    print(v_id)
