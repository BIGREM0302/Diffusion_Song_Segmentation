...
if __name__ == "__main__":
    rolls = np.load("generated_song.npy")
    number_of_song = rolls.shape[0]
    exp_path = os.path.join(PROJECT_PATH, 'midi_a')
    os.makedirs(exp_path, exist_ok=True)
    bpm = 120
    unit = 0.25
    print(f"There is {number_of_song} rolls waited to be converted")
    for i in range(number_of_song):
        note_mat = piano_roll_to_note_mat(rolls[i], False)
        notes = note_mat_to_notes(note_mat, bpm, unit)
        midi = create_pm_object(bpm, preset=0, notes_list=[notes])
