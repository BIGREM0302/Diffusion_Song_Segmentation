...
class DiffproLearner:
    def __init__(
        self, output_dir, model, train_dl, val_dl, optimizer, params, param_scheduler, use_wandb = True
    ):
        self.output_dir = output_dir
        # log_dir will carry log information
        self.log_dir = f"{output_dir}/logs"
        # checkpoint_dir will carry checkpoint information
        self.checkpoint_dir = f"{output_dir}/chkpts"
        # model: Diffpro_SDF class
        self.model = model
        # the data loader of train set
        self.train_dl = train_dl
        # the data loader of validation set
        self.val_dl = val_dl
        # the adam optimizer
        ...
        self.autocast = torch.amp.autocast("cuda", enabled=params.fp16)
        # to prevent gradient disappear, during bakc propogation always scale up, but need to scale down when updating
        self.scaler = torch.amp.GradScaler("cuda", enabled=params.fp16)
        ...
        if os.path.exists(self.output_dir):
            # check if the output_dir/chkpts/weights.pt exist, if exist then return true and load the parameters
            self.restore_from_checkpoint()
        else:
            os.makedirs(self.output_dir)
            os.makedirs(self.log_dir)
            os.makedirs(self.checkpoint_dir)
            # write self.params into output_dir/params.json
        ...
        if self.use_wandb:
            wandb.init(
                project="diff-song-seg",
                config=params.__dict__ if hasattr(params, "__dict__") else params,
                name=f"exp_{output_dir.split('/')[-1]}",
                dir=output_dir
            )
    ...
    def _write_summary(self, losses: dict, scheduled_params: Optional[dict], type):
        ...
        if self.use_wandb:
            wandb.log({f"{type}/{k}": v for k, v in summary_losses.items()}, step=self.step)

    def state_dict(self):
        model_state = self.model.state_dict()
        # scaler / optimizer / model all have state_dict() methods
        return {
            "step": self.step,
            "epoch": self.epoch,
            "model":
    ...
    def valid(self):
        ...
        if self.use_wandb:
            ...
            wandb.log({"val/loss": losses["loss"]}, step=self.step)
